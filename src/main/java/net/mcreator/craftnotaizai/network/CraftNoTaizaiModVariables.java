	...
	public static class PlayerVariables {
		...
		public void syncPlayerVariables(Entity entity) {
			if (entity instanceof ServerPlayer serverPlayer)
				CraftNoTaizaiMod.PACKET_HANDLER.send(PacketDistributor.PLAYER.with(() -> serverPlayer), new PlayerVariablesSyncMessage(this));
		}

		public Tag writeNBT() {
			CompoundTag nbt = new CompoundTag();
			nbt.putDouble("level", level);
			nbt.putDouble("levelxp", levelxp);
			nbt.putDouble("maxlevelxp", maxlevelxp);
			return nbt;
		}
		...
		public void readNBT(Tag Tag) {
			...
			levelxp = nbt.getDouble("levelxp");
			maxlevelxp = nbt.getDouble("maxlevelxp");
		}
	}
		...
		public static void handler(PlayerVariablesSyncMessage message, Supplier<NetworkEvent.Context> contextSupplier) {
			...
			context.enqueueWork(() -> {
				if (!context.getDirection().getReceptionSide().isServer()) {
					...
					variables.levelxp = message.data.levelxp;
					variables.maxlevelxp = message.data.maxlevelxp;
				}
			});
